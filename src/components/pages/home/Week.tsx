...
const Week = () => {
  ...
  const [currentWeekArr, setCurrentWeekArr] = useState<number[]>(weekArr)
  ...
  return (
    <div className={styles.week_wrapper}>
      <div className={`${styles.select_wrapper} b-small`}>
        <p>{nowYear}.{String(nowMonth + 1).padStart(2, "0")}</p>
        <div onClick={onToggle} className={styles.dropdown_wrapper}>
          <div className={styles.dropdown_line}>
          {selectedWeek}주차 <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5" width="17">
...

          {
            isOpen && (
              ...
              {new Array(totalWeek).fill(null).map((_, idx)=>(
                <div key={`week-${idx}`} onClick={()=>onClickWeek(idx+1)} className={idx+1 === selectedWeek ? styles.active : ""}>{idx+1}주차</div>
              ))}
              </div>
            )
          }
      ...
      <div className={styles.week}>
        {currentWeekArr.map((date, idx) => (
          <div className={styles.date_wrapper} key={idx}>
            {idx === 0 ? <div className={`${styles.sunday} r-small`}>{week[idx]}</div> : idx === 6 ? <div className={`${styles.saturday} r-small`}>{week[idx]}</div> : <div className={`${styles.day} r-small`}>{week[idx]}</div>}
            <div className={styles.date} onClick={()=>handleClick(idx)}>
              {(date === 0) ? <></> : (selectedDay === idx) ? <div className={`${styles.selected} r-large`}>{date}</div> : <>{date}</>}
            </div>
          </div>
        ))}
      </div>
      <div className={styles.today}>
        <p className={`${styles.title} b-regular`}>{(selectedNow && currentWeekArr[selectedDay] === nowDate) ? "오늘의" : `${week[selectedDay]}요일`} 식단</p>
        <Calorie
        ...
        />
        <MealCard 
          dateArr={DUMMYdayData.dateArr as [string, number, string | undefined][]}
        />
      </div>
    </div>
  );
