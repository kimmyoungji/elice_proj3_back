...
export class UserController {
  ...
  public async handleGetUserInfo(@Req() request: any, @Res() response: any) {
    const result = await this.userService.getUserAndHealthInfo(
      request.user.userId
    );
  ...
  @ApiResponse({
    status: 204,
    description: "유저정보와 유저건강정보 수정 성공.",
  })
  ...
  public async updateUserInfo(
    ...
    const result = await this.userService.updateUserAndHealthInfo(
      request.user.userId,
      updateUserAndHealthInfoDto
    );
